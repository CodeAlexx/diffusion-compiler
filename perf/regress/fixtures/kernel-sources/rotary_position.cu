...
extern "C" __global__ void dif_op_1(const dif_f32* positions, const dif_f32* inv_freq, dif_bf16* cosine, dif_bf16* sine) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 48ULL) {
    unsigned long long row = i / 12ULL, column = i % 12ULL,
                       component = column % 6ULL,
                       axis = component / 3ULL, frequency = component % 3ULL;
    float angle = dif_load_f32(positions, row * 2ULL + axis) * dif_load_f32(inv_freq, frequency);
    dif_store_bf16(cosine, i, cosf(angle));
    dif_store_bf16(sine, i, sinf(angle));
  }
}
