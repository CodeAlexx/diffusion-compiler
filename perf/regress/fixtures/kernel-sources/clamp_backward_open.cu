...
extern "C" __global__ void dif_op_1(const dif_bf16* g, const dif_bf16* x, dif_bf16* y) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 32ULL) {
    float v = dif_load_bf16(x, i);
    dif_store_bf16(y, i, (v >= -3.402823466e+38f && v <= 3.402823466e+38f) ? dif_load_bf16(g, i) : 0.0f);
  }
}
#undef dif_scalar
#undef dif_load
