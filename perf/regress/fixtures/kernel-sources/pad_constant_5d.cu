...
// Constant padding of an NCDHW tensor (front/back on the depth axis).
extern "C" __global__ void dif_op_1(const dif_scalar* x, dif_scalar* y) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 120ULL) {
    unsigned long long ow = i % 6ULL, oh = (i / 6ULL) % 5ULL,
                       ot = (i / 30ULL) % 4ULL, c = (i / 120ULL) % 1ULL,
                       b = i / 120ULL;
    if (ot < 1ULL || ot >= 3ULL || oh < 1ULL || oh >= 4ULL ||
        ow < 2ULL || ow >= 5ULL) {
      dif_store(y, i, 0.000000000e+00f);
      return;
    }
    unsigned long long source = (((b * 1ULL + c) * 2ULL + ot - 1ULL) * 3ULL + oh - 1ULL) * 3ULL + ow - 2ULL;
