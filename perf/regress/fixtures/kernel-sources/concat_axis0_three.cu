...
extern "C" __global__ void dif_op_1(const dif_scalar* input0, const dif_scalar* input1, const dif_scalar* input2, dif_scalar* output) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 32ULL) {
    unsigned long long inner_index = i % 8ULL,
                       axis_coordinate = (i / 8ULL) % 4ULL,
                       outer = i / (4ULL * 8ULL), source = 0ULL;
    if (axis_coordinate < 1ULL) {
      source = (outer * 1ULL + (axis_coordinate - 0ULL)) * 8ULL + inner_index;
      dif_store(output, i, dif_load(input0, source));
    }
    else if (axis_coordinate < 3ULL) {
      source = (outer * 2ULL + (axis_coordinate - 1ULL)) * 8ULL + inner_index;
      dif_store(output, i, dif_load(input1, source));
    }
    else if (axis_coordinate < 4ULL) {
      source = (outer * 1ULL + (axis_coordinate - 3ULL)) * 8ULL + inner_index;
      dif_store(output, i, dif_load(input2, source));
    }
