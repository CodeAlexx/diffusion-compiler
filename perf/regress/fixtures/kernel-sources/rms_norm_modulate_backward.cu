...
extern "C" __global__ void dif_op_1(const dif_scalar* grad_output, const dif_scalar* x, const dif_scalar* scale, dif_scalar* grad_input, dif_scalar* grad_scale, dif_scalar* grad_shift) {
  ...
  if (i < 32ULL) {
    ...
    float dot = 0.0f;
    for (unsigned long long k = 0ULL; k < 8ULL; ++k)
      dot = fmaf(dif_load(grad_output, base + k) * (1.0f + dif_load(scale, base + k)), dif_load(x, base + k), dot);
