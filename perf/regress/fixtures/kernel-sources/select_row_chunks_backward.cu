...
extern "C" __global__ void dif_op_1(const int* indices, const dif_bf16* g0, const dif_bf16* g1, dif_bf16* grad_values) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 30ULL) {
    unsigned long long row = i / 6ULL, col = i % 6ULL;
    unsigned long long chunk = col / 3ULL, offset = col % 3ULL;
    const dif_bf16* source = chunk == 0ULL ? g0 : g1;
