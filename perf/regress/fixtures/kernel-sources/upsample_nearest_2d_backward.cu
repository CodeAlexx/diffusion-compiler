...
extern "C" __global__ void dif_op_1(const dif_scalar* g, dif_scalar* y) {
  ...
  if (i < 12ULL) {
    unsigned long long x = i % 2ULL;
    unsigned long long h = (i / 2ULL) % 3ULL;
    unsigned long long plane = i / (2ULL * 3ULL);
    float total = 0.0f;
    for (unsigned long long dy = 0ULL; dy < 2ULL; ++dy)
      for (unsigned long long dx = 0ULL; dx < 4ULL; ++dx)
        total += dif_load(g, (plane * 6ULL + h * 2ULL + dy) * 8ULL + x * 4ULL + dx);
    dif_store(y, i, total);
  }
}
#undef dif_scalar
