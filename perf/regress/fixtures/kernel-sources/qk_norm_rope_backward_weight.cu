...
extern "C" __global__ void dif_op_1(const dif_scalar* grad_output, const dif_scalar* x, const dif_scalar* weight, const dif_bf16* cosv, const dif_bf16* sinv, dif_scalar* grad_input, dif_scalar* grad_weight) {
  ...
  if (i < 64ULL) {
    ...
    dif_store(grad_input, i, own_rotated * dif_load(weight, d) * inv - value * inv * inv * inv * dot / 8.0f);
        if (i < 8ULL) {
      float acc = 0.0f;
      for (unsigned long long r = 0ULL; r < 8ULL; ++r) {
        unsigned long long rrb = r * 8ULL, token = r / 2ULL, rtb = token * 8ULL;
        float rss = 0.0f;
        for (unsigned long long k = 0ULL; k < 8ULL; ++k) {
        ...
        float rotated_gradient = (i < 4ULL ? dif_load(grad_output, rrb + i) * dif_load_bf16(cosv, rtb + i) + dif_load(grad_output, rrb + i + 4ULL) * dif_load_bf16(sinv, rtb + i + 4ULL) : (i < 8ULL ? -dif_load(grad_output, rrb + i - 4ULL) * dif_load_bf16(sinv, rtb + i - 4ULL) + dif_load(grad_output, rrb + i) * dif_load_bf16(cosv, rtb + i) : dif_load(grad_output, rrb + i)));
