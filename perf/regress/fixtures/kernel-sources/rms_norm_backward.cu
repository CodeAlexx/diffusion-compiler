...
extern "C" __global__ void dif_op_1(const dif_scalar* grad_output,const dif_scalar* x,const dif_scalar* weight,dif_scalar* grad_input){unsigned long long i=(unsigned long long)blockIdx.x*blockDim.x+threadIdx.x;if(i<32ULL){unsigned long long row=i/8ULL,base=row*8ULL;float ss=0.0f;for(unsigned long long k=0ULL;k<8ULL;++k){float value=dif_load(x,base+k);ss=fmaf(value,value,ss);}float inv=rsqrtf(ss/8.0f+9.999999975e-07f);float dot=0.0f;for(unsigned long long k=0ULL;k<8ULL;++k)dot=fmaf(dif_load(grad_output,base+k)*dif_load(weight,k),dif_load(x,base+k),dot);float value=dif_load(x,i);float gradient=dif_load(grad_output,i)*dif_load(weight,i-base)*inv-value*inv*inv*inv*dot/8.0f;dif_store(grad_input,i,gradient);}}
#undef dif_scalar
#undef dif_load
#undef dif_store
#undef dif_round
