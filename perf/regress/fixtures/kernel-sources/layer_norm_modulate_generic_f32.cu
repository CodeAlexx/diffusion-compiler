...
extern "C" __global__ void dif_op_1(const dif_scalar* x, const dif_scalar* weight, const dif_scalar* bias, const dif_scalar* scale, const dif_scalar* shift, dif_scalar* y) {
  ...
  if (row >= 4ULL) return;
  float local = 0.0f;
  for (unsigned long long col = threadIdx.x; col < 8ULL; col += blockDim.x) local += dif_load(x, row * 8ULL + col);
  ...
  for (unsigned long long col = threadIdx.x; col < 8ULL; col += blockDim.x) {
    float centered = dif_load(x, row * 8ULL + col) - mean;
  ...
  float inv = rsqrtf(reduction[0] / 8.0f + 9.9999997473787516e-06f);
  unsigned long long modulation_row = row / 1ULL;
  for (unsigned long long col = threadIdx.x; col < 8ULL; col += blockDim.x) {
    unsigned long long i = row * 8ULL + col, mi = modulation_row * 8ULL + col;
