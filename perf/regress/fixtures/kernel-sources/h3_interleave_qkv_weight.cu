...
// nothing is dropped.
extern "C" __global__ void dif_op_1(const dif_bf16* grad_q, const dif_bf16* grad_k, const dif_bf16* grad_v, dif_bf16* grad_packed) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 72ULL) {
    unsigned long long column = i % 4ULL, rest = i / 4ULL;
    unsigned long long d = rest % 3ULL, rest2 = rest / 3ULL;
    unsigned long long component = rest2 % 3ULL, head = rest2 / 3ULL;
    const dif_bf16* source = component == 0ULL ? grad_q : component == 1ULL ? grad_k : grad_v;
    dif_store_bf16(grad_packed, i,
             dif_load_bf16(source, (head * 3ULL + d) * 4ULL + column));
  }
}
#undef dif_scalar
#undef dif_load
