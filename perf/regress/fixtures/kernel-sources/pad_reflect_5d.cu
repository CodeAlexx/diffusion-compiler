...
extern "C" __global__ void dif_op_1(const dif_scalar* x, dif_scalar* y) {
  ...
  if (i < 180ULL) {
    unsigned long long ow = i % 6ULL, oh = (i / 6ULL) % 6ULL,
                       ot = (i / 36ULL) % 5ULL, c = (i / 180ULL) % 1ULL,
                       b = i / 180ULL;
    unsigned long long st = ot < 1ULL ? 1ULL - ot : (ot - 1ULL < 3ULL ? ot - 1ULL : 2ULL * 3ULL - 2ULL - (ot - 1ULL));
    unsigned long long sy = oh < 1ULL ? 1ULL - oh : (oh - 1ULL < 4ULL ? oh - 1ULL : 2ULL * 4ULL - 2ULL - (oh - 1ULL));
    unsigned long long sx = ow < 1ULL ? 1ULL - ow : (ow - 1ULL < 4ULL ? ow - 1ULL : 2ULL * 4ULL - 2ULL - (ow - 1ULL));
    unsigned long long source = (((b * 1ULL + c) * 3ULL + st) * 4ULL + sy) * 4ULL + sx;
