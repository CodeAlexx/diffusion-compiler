...
extern "C" __global__ void dif_op_1(const dif_bf16* grad_output, const dif_bf16* q, const dif_bf16* k, const dif_bf16* v, const dif_bf16* forward_output, const dif_f32* lse, dif_bf16* grad_q, dif_bf16* grad_k, dif_bf16* grad_v) {
  ...
  if (i < 128ULL) {
    ...
    float dq = 0.0f;
    {
      unsigned long long kh = h / 2ULL, kend = 6ULL;
      ...
      for (unsigned long long ks = 0ULL; ks < kend; ++ks) {
        unsigned long long kb = ((row / 8ULL * 6ULL + ks) * 1ULL + kh) * 8ULL;
  ...
  if (i < 96ULL) {
    unsigned long long krow = i / 8ULL, d = i % 8ULL, ks = krow / 1ULL % 6ULL, h = krow % 1ULL, kvb = krow * 8ULL;
    ...
    for (unsigned long long g = 0ULL; g < 2ULL; ++g) {
      ...
      for (unsigned long long qs = 0ULL; qs < 4ULL; ++qs) {
        unsigned long long qb = ((krow / 6ULL * 4ULL + qs) * 2ULL + qh) * 8ULL;
        float row_lse = dif_load_f32(lse, (krow / 6ULL * 4ULL + qs) * 2ULL + qh);
