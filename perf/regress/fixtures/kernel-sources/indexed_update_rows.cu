...
extern "C" __global__ void dif_op_1(const dif_scalar* base, const dif_scalar* updates, const int* map, dif_scalar* y) {
  ...
  if (i < 40ULL) {
    ...
    if (source == -1)
      ...
      dif_store(y, i, dif_load(updates, (unsigned long long)source * 8ULL + col));
    else
