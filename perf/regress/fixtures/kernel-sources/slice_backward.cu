...
extern "C" __global__ void dif_op_1(const dif_scalar* g, dif_scalar* y) {
  ...
  if (i < 60ULL) {
    unsigned long long lane = i % 4ULL;
    unsigned long long position = (i / 4ULL) % 5ULL;
    unsigned long long outer = i / (4ULL * 5ULL);
    if (position >= 2ULL && position < 2ULL + 2ULL)
      dif_store(y, i, dif_load(g, (outer * 2ULL + (position - 2ULL)) * 4ULL + lane));
    else
      dif_store(y, i, 0.0f);
  }
}
