...
extern "C" __global__ void dif_op_2(const dif_bf16* a0, const dif_bf16* a1, dif_bf16* y) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 32ULL) {
    float s0=dif_round_bf16(dif_load_bf16(a0,i)+dif_load_bf16(a1,i));asm("" : "+f"(s0));
    dif_store_bf16(y, i, dif_silu(s0));
  }
}
