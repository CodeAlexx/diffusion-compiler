...
extern "C" __global__ void dif_op_1(const dif_scalar* input, dif_scalar* output) {
  ...
  if (i < 24ULL) {
    ...
    coordinate = remaining % 3ULL; remaining /= 3ULL; source += coordinate * 4ULL;
    coordinate = remaining % 2ULL; remaining /= 2ULL; source += coordinate * 12ULL;
    coordinate = remaining % 4ULL; remaining /= 4ULL; source += coordinate * 1ULL;
