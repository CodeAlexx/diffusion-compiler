...
extern "C" __global__ void dif_op_1(const dif_scalar* sample, const dif_scalar* velocity, const dif_f32* timesteps, const dif_f32* sigmas, dif_scalar* output) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 32ULL) {
    float timestep = dif_load_f32(timesteps, 1ULL), sigma = dif_load_f32(sigmas, 1ULL),
          sigma_next = dif_load_f32(sigmas, 2ULL), sample_value = dif_load(sample, i),
