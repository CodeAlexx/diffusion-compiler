...
extern "C" __global__ void dif_op_1(const dif_bf16* x0,signed char* q, float* scales) {
  ...
  for (unsigned long long column = tid; column < 512ULL; column += 256ULL) {
    float value = dif_load_bf16(x0,row*512ULL+column-0ULL);
  ...
  for (unsigned stage = 0U; stage < 4U; ++stage) {
    ...
    for (unsigned long long tuple = tid; tuple < 512ULL / 4ULL; tuple += 256ULL) {
      unsigned long long group = (tuple / 64ULL) * 256ULL;
      unsigned lane = (unsigned)(tuple % 64ULL);
