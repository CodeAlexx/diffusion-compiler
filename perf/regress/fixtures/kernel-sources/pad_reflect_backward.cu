...
extern "C" __global__ void dif_op_1(const dif_scalar* grad_output, dif_scalar* grad_input) {
  ...
  if (i < 60ULL) {
    ...
    rest /= 5ULL;
    unsigned long long t = rest % 1ULL, plane = rest / 1ULL;
    unsigned long long ot[3], oh[3], ow[3];
    int nt = 0, nh = 0, nw = 0;
    ot[nt++] = t + 0ULL;
    if (t >= 1ULL && t <= 0ULL) ot[nt++] = 0ULL - t;
    if (0ULL >= 1ULL && t + 1ULL + 0ULL >= 1ULL && t + 2ULL <= 1ULL) ot[nt++] = 0ULL + 2ULL * 1ULL - 2ULL - t;
    oh[nh++] = h + 1ULL;
    ...
    for (int a = 0; a < nt; ++a)
      for (int b = 0; b < nh; ++b)
        for (int c = 0; c < nw; ++c)
          accumulator += dif_load(grad_output, ((plane * 1ULL + ot[a]) * 9ULL + oh[b]) * 10ULL + ow[c]);
