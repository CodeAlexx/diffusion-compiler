...
extern "C" __global__ void dif_op_1(const dif_scalar* x, const dif_scalar* weight, dif_scalar* y) {
  ...
  if (threadIdx.x < 128U) {
    for (unsigned long long pack = threadIdx.x; pack < 32ULL; pack += 128ULL) {
      unsigned long long col = pack * 4ULL;
      unsigned long long base = row * 128ULL + col;
  ...
  float inv = rsqrtf(reduction[0] / 128.0f + 9.9999999747524271e-07f);
