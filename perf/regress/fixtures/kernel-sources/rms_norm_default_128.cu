...
extern "C" __global__ void dif_op_1(const dif_scalar* x,const dif_scalar* weight,dif_scalar* y){
  ...
  if(threadIdx.x<128U){for(unsigned long long pack=threadIdx.x;pack<32ULL;pack+=128ULL){unsigned long long col=pack*4ULL;unsigned long long base=row*128ULL+col;float v0=dif_load(x,base);local+=v0*v0;float v1=dif_load(x,base+1ULL);local+=v1*v1;float v2=dif_load(x,base+2ULL);local+=v2*v2;float v3=dif_load(x,base+3ULL);local+=v3*v3;}}
  ...
  float inv=rsqrtf(reduction[0]/128.0f+9.9999999747524271e-07f);for(unsigned long long col=threadIdx.x;col<128ULL;col+=blockDim.x){unsigned long long i=row*128ULL+col;dif_store(y,i,dif_load(x,i)*inv*(dif_load(weight,col)+0.000000000e+00f));}}
#undef dif_scalar
#undef dif_load
#undef dif_store
#undef dif_round
