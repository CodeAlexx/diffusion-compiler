...
extern "C" __global__ void dif_op_1(const unsigned char* mask, dif_bf16* y) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 32ULL) {
    unsigned long long key = i % 4ULL, query = (i / 4ULL) % 4ULL,
                       batch = i / (4ULL * 4ULL);
    bool valid = mask[batch * 4ULL + query] && mask[batch * 4ULL + key];
    dif_store_bf16(y, i, valid ? 0.0f : -__int_as_float(0x7f800000));
  }
}
