...
extern "C" __global__ void dif_op_1(const dif_scalar* input0, const dif_scalar* input1, dif_scalar* output) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 32ULL) {
    unsigned long long inner_index = i % 1ULL,
                       axis_coordinate = (i / 1ULL) % 8ULL,
                       outer = i / (8ULL * 1ULL), source = 0ULL;
    if (axis_coordinate < 3ULL) {
      source = (outer * 3ULL + (axis_coordinate - 0ULL)) * 1ULL + inner_index;
      dif_store(output, i, dif_load(input0, source));
    }
    else if (axis_coordinate < 8ULL) {
      source = (outer * 5ULL + (axis_coordinate - 3ULL)) * 1ULL + inner_index;
