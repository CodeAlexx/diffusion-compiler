...
struct dif_welford_1 { float mean; float sigma2; float count; };
extern "C" __device__ __forceinline__ dif_welford_1 dif_welford_online_1(float value, dif_welford_1 current) {
...
extern "C" __device__ __forceinline__ dif_welford_1 dif_welford_combine_1(dif_welford_1 data_b, dif_welford_1 data_a) {
...
extern "C" __global__ void dif_op_1(const dif_scalar* x, const dif_scalar* weight, const dif_scalar* bias, dif_scalar* y) {
  ...
  dif_welford_1 state = {0.0f, 0.0f, 0.0f};
  for (unsigned long long pack = tid; pack < 32ULL; pack += 128ULL) {
    unsigned long long base = row * 128ULL + pack * 4ULL;
    state = dif_welford_online_1(dif_load(x, base), state);
    state = dif_welford_online_1(dif_load(x, base + 1ULL), state);
    state = dif_welford_online_1(dif_load(x, base + 2ULL), state);
    state = dif_welford_online_1(dif_load(x, base + 3ULL), state);
  }
  for (int offset = 16; offset > 0; offset >>= 1) {
    dif_welford_1 other = {__shfl_down_sync(0xffffffffU, state.mean, offset),
                                   __shfl_down_sync(0xffffffffU, state.sigma2, offset),
                                   __shfl_down_sync(0xffffffffU, state.count, offset)};
    state = dif_welford_combine_1(state, other);
  }
  ...
  for (unsigned offset = 2U; offset > 0U; offset >>= 1U) {
    ...
    if (lane == 0U && warp < offset) {
      dif_welford_1 other = {meansigma[2U * warp], meansigma[2U * warp + 1U], counts[warp]};
      state = dif_welford_combine_1(state, other);
    }
    __syncthreads();
  }
  if (tid == 0U) {
    ...
    meansigma[1] = state.sigma2 / 128.0f;
  }
  __syncthreads();
  float mean = meansigma[0];
  float inverse = rsqrtf(meansigma[1] + 9.9999997473787516e-06f);
  for (unsigned long long pack = tid; pack < 32ULL; pack += 128ULL) {
    unsigned long long base = row * 128ULL + pack * 4ULL;
