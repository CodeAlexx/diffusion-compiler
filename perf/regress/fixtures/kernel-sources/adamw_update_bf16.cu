...
extern "C" __global__ void dif_op_1(const dif_bf16* parameter, const dif_f32* gradient, const dif_f32* first, const dif_f32* second, const int* completed_steps, dif_bf16* updated, dif_f32* updated_first, dif_f32* updated_second) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 8ULL) {
    float step = (float)(completed_steps[0] + 1), beta1 = 8.999999762e-01f, beta2 = 9.990000129e-01f;
    float grad = dif_load_f32(gradient, i) * 5.000000000e-01f;
