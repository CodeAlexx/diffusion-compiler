...
extern "C" __global__ void dif_op_1(const dif_bf16* x, const dif_f32* cosine, const dif_f32* sine, dif_bf16* y) {
  ...
  if (i < 64ULL) {
    ...
    if (d < 8ULL) {
      unsigned long long pair = d % 4ULL, base = i - d, table = (batch * 4ULL + token) * 4ULL + pair;
      float even = dif_load_bf16(x, base + pair), odd = dif_load_bf16(x, base + pair + 4ULL), c = dif_load_f32(cosine, table),
