...
extern "C" __global__ void dif_op_1(const dif_scalar* grad_output, const dif_scalar* x, const dif_scalar* gamma, dif_scalar* grad_input, dif_scalar* grad_gamma) {
  ...
  if (i < 64ULL) {
    ...
    for (unsigned long long f = 0ULL; f < 16ULL; ++f) {
      unsigned long long fiber = f / 8ULL * 4ULL * 8ULL + f % 8ULL;
      float squared = 0.0f;
      for (unsigned long long c = 0ULL; c < 4ULL; ++c) {
        float value = dif_load(x, fiber + c * 8ULL);
        squared = fmaf(value, value, squared);
      }
      float denominator = fmaxf(sqrtf(squared), 9.999999960e-13f);
      unsigned long long index = fiber + i * 8ULL;
      accumulator = fmaf(dif_load(grad_output, index),
                         dif_load(x, index) * 2.000000000e+00f / denominator,
                         accumulator);
    }
