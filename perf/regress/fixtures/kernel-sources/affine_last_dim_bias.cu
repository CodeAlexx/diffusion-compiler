...
extern "C" __global__ void dif_op_1(const dif_scalar* x,const dif_scalar* scale,const dif_scalar* bias,dif_scalar* y){unsigned long long i=(unsigned long long)blockIdx.x*blockDim.x+threadIdx.x;if(i<32ULL){unsigned long long col=i%8ULL;float value=dif_round(dif_load(x,i)*dif_load(scale,col));value=dif_round(value+dif_load(bias,col));dif_store(y,i,value);}}
#undef dif_scalar
#undef dif_load
#undef dif_store
#undef dif_round
