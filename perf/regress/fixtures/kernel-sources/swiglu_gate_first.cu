...
extern "C" __global__ void dif_op_1(const dif_scalar* x, dif_scalar* y) {
  ...
  if(i<32ULL){ unsigned long long row=i/8ULL, col=i%8ULL; float value=dif_load(x,row*16ULL+8ULL+col); float gate=dif_load(x,row*16ULL+0ULL+col); float activated=dif_round(dif_silu(gate));dif_store(y,i,value*activated);}
}
#undef dif_scalar
#undef dif_load
#undef dif_store
