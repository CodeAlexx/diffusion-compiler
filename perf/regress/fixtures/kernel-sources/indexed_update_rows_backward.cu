...
extern "C" __global__ void dif_op_1(const dif_bf16* grad_output, const int* map, dif_bf16* grad_base, dif_bf16* grad_updates) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 20ULL) {
    unsigned long long row = i / 4ULL;
    dif_store_bf16(grad_base, i, map[row] < 0 ? dif_load_bf16(grad_output, i) : 0.0f);
  }
    if (i < 12ULL) {
    unsigned long long row = i / 4ULL, column = i % 4ULL;
    float accumulator = 0.0f;
    for (unsigned long long r = 0ULL; r < 5ULL; ++r)
      if (map[r] >= 0 && (unsigned long long)map[r] == row)
        accumulator += dif_load_bf16(grad_output, r * 4ULL + column);
    dif_store_bf16(grad_updates, i, accumulator);
  }
