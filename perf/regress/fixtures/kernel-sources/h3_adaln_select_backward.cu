...
extern "C" __global__ void dif_op_1(const int* indices, const dif_bf16* g0, const dif_bf16* g1, const dif_bf16* g2, const dif_bf16* g3, const dif_bf16* g4, const dif_bf16* g5, dif_bf16* grad_projected) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 108ULL) {
    unsigned long long col = i % 3ULL, rest = i / 3ULL;
    unsigned long long chunk = rest % 6ULL, table = rest / 6ULL;
    const dif_bf16* source = chunk == 0ULL ? g0 : chunk == 1ULL ? g1 : chunk == 2ULL ? g2 : chunk == 3ULL ? g3 : chunk == 4ULL ? g4 : g5;
    float accumulator = 0.0f;
    for (unsigned long long s = 0ULL; s < 4ULL; ++s)
      if ((unsigned long long)indices[s] == table)
        accumulator += dif_load_bf16(source, s * 3ULL + col);
    dif_store_bf16(grad_projected, i, accumulator);
  }
}
#undef dif_scalar
#undef dif_load
