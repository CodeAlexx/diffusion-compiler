...
extern "C" __global__ void dif_op_1(const dif_scalar* x, const dif_scalar* w, dif_scalar* y) {
  ...
  if (i < 40ULL) {
    unsigned long long o = i % 10ULL;
    unsigned long long oc = (i / 10ULL) % 4ULL;
    unsigned long long b = i / 40ULL;
    unsigned long long group = oc / 4ULL;
    float acc = 0.0f;
    unsigned long long ocg = oc % 4ULL;
    long long ofull = (long long)o + 0LL;
    for (unsigned long long ic = 0; ic < 2ULL; ++ic) {
      const dif_scalar* xrow = x + ((b * 2ULL) + (group * 2ULL + ic)) * 4ULL;
      const dif_scalar* wrow = w + (((group * 2ULL + ic) * 4ULL) + ocg) * 4ULL;
      long long imin = (ofull - 3LL + 2LL - 1LL) / 2LL;
      if (imin < 0LL) imin = 0LL;
      long long imax = ofull / 2LL;
      if (imax > 3LL) imax = 3LL;
      for (long long pi = imin; pi <= imax; ++pi) {
        long long k = ofull - pi * 2LL;
        if (k < 0LL || k >= 4LL) continue;
        acc += (((pi - 0LL) >= 0LL && (pi - 0LL) < 4LL) ? dif_load(xrow, (unsigned long long)(pi - 0LL)) : 0.0f) * dif_load(wrow, (unsigned long long)k);
      }
    }
