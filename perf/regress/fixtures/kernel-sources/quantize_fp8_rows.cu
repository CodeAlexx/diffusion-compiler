...
extern "C" __global__ void dif_op_1(const dif_bf16* x, unsigned char* q, float* scales) {
  __shared__ float maximums[256];
  unsigned long long row = blockIdx.x;
  unsigned tid = threadIdx.x;
  if (row >= 4ULL) return;
  unsigned long long base = row * 64ULL;
  float maximum = 0.0f;
  for (unsigned long long column = tid; column < 64ULL; column += 256ULL) {
