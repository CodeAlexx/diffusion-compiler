...
extern "C" __global__ void dif_op_1(const dif_scalar* residual, const dif_scalar* branch, const dif_scalar* gate, dif_scalar* y) {
  unsigned long long i=(unsigned long long)blockIdx.x*blockDim.x+threadIdx.x; if(i<32ULL){unsigned long long row=i/8ULL,gate_index=(row/4ULL)*8ULL+i%8ULL;dif_store(y,i,dif_load(residual,i)+dif_round(dif_load(gate,gate_index)*dif_load(branch,i)));}
}
#undef dif_scalar
#undef dif_load
#undef dif_store
