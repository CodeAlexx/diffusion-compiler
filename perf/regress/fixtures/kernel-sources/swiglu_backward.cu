...
extern "C" __global__ void dif_op_1(const dif_scalar* grad_output, const dif_scalar* x, dif_scalar* grad_input) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 64ULL) {
    unsigned long long row = i / 16ULL, col = i % 16ULL;
    if (col < 0ULL || col >= 16ULL) {
      dif_store(grad_input, i, 0.0f);
      return;
    }
    unsigned long long lane = col - 0ULL, cw = lane < 8ULL ? lane : lane - 8ULL,
                       base = row * 16ULL + 0ULL;
    float value = dif_load(x, base + 0ULL + cw);
    float gate = dif_load(x, base + 8ULL + cw);
    float sigmoid = 1.0f / (1.0f + expf(-gate));
    float upstream = dif_load(grad_output, row * 8ULL + cw);
    int is_value_slot = lane < 8ULL;
