...
extern "C" __global__ void dif_op_1(const dif_scalar* grad_output, const dif_scalar* x, const dif_scalar* weight, const dif_bf16* cosv, const dif_bf16* sinv, dif_scalar* grad_input) {
  ...
  if (i < 64ULL) {
    unsigned long long row = i / 8ULL, d = i % 8ULL, rb = row * 8ULL, token = row / 2ULL, tb = token * 4ULL;
    float ss = 0.0f;
    ...
    float dot = 0.0f;
    for (unsigned long long k = 0ULL; k < 8ULL; ++k) {
      float rotated_gradient = (k < 4ULL ? dif_load(grad_output, rb + k) * dif_load_bf16(cosv, tb + k) + dif_load(grad_output, rb + k + 4ULL) * dif_load_bf16(sinv, tb + k) : (k < 8ULL ? -dif_load(grad_output, rb + k - 4ULL) * dif_load_bf16(sinv, tb + k - 4ULL) + dif_load(grad_output, rb + k) * dif_load_bf16(cosv, tb + k - 4ULL) : dif_load(grad_output, rb + k)));
      dot = fmaf(rotated_gradient * dif_load(weight, k), dif_load(x, rb + k), dot);
    }
    float own_rotated = (d < 4ULL ? dif_load(grad_output, rb + d) * dif_load_bf16(cosv, tb + d) + dif_load(grad_output, rb + d + 4ULL) * dif_load_bf16(sinv, tb + d) : (d < 8ULL ? -dif_load(grad_output, rb + d - 4ULL) * dif_load_bf16(sinv, tb + d - 4ULL) + dif_load(grad_output, rb + d) * dif_load_bf16(cosv, tb + d - 4ULL) : dif_load(grad_output, rb + d)));
