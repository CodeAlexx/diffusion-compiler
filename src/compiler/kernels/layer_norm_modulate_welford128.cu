...
extern "C" __device__ __forceinline__ dif_welford_mod_${suffix} dif_welford_mod_online_${suffix}(float value, dif_welford_mod_${suffix} current) {
...
extern "C" __device__ __forceinline__ dif_welford_mod_${suffix} dif_welford_mod_combine_${suffix}(dif_welford_mod_${suffix} data_b, dif_welford_mod_${suffix} data_a) {
...
extern "C" __global__ void ${function}(const dif_scalar* x, const dif_scalar* weight, const dif_scalar* bias, const dif_scalar* scale, const dif_scalar* shift, dif_scalar* y) {
  ...
  dif_welford_mod_${suffix} state = {0.0f, 0.0f, 0.0f};
  for (unsigned long long pack = tid; pack < ${packs}ULL; pack += 128ULL) {
    unsigned long long base = row * ${columns}ULL + pack * 4ULL;
    state = dif_welford_mod_online_${suffix}(dif_load(x, base), state);
    state = dif_welford_mod_online_${suffix}(dif_load(x, base + 1ULL), state);
    state = dif_welford_mod_online_${suffix}(dif_load(x, base + 2ULL), state);
    state = dif_welford_mod_online_${suffix}(dif_load(x, base + 3ULL), state);
  }
  for (int offset = 16; offset > 0; offset >>= 1) {
    dif_welford_mod_${suffix} other = {__shfl_down_sync(0xffffffffU, state.mean, offset), __shfl_down_sync(0xffffffffU, state.sigma2, offset), __shfl_down_sync(0xffffffffU, state.count, offset)};
    state = dif_welford_mod_combine_${suffix}(state, other);
  }
  ...
  for (unsigned offset = 2U; offset > 0U; offset >>= 1U) {
    ...
    if (lane == 0U && warp < offset) {
      dif_welford_mod_${suffix} other = {meansigma[2U * warp], meansigma[2U * warp + 1U], counts[warp]};
      state = dif_welford_mod_combine_${suffix}(state, other);
    }
    __syncthreads();
  }
  ...
  for (unsigned long long pack = tid; pack < ${packs}ULL; pack += 128ULL) {
    ...
    for (unsigned inner = 0U; inner < 4U; ++inner) {
      unsigned long long column = pack * 4ULL + inner, index = base + inner, modulation_index = modulation_row * ${columns}ULL + column;
