...
// Keep F64 intermediates, as in the CPU semantic implementation and creator
// rope(). Only the completed tables round to F32; early rounding changes Q/K.
extern "C" __global__ void ${function}(const dif_f32* positions, const int* pair_axes, const int* pair_indices, const int* axis_dims, dif_f32* cosine, dif_f32* sine) {
  ...
  if (i < ${count}ULL) {
    ...
    double scale = (2.0 * (double)component) / (double)axis_dim;
    double omega = 1.0 / pow((double)${theta} * (double)${ntk}, scale);
    double angle = (double)dif_load_f32(positions, (batch * ${sequence}ULL + token) * ${axes}ULL + (unsigned long long)axis) * omega;
    dif_store_f32(cosine, i, (float)cos(angle));
    dif_store_f32(sine, i, (float)sin(angle));
  }
}
