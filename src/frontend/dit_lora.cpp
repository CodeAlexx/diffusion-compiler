...
make_dit_lora_training(const DitLoraTrainingConfig &config) {
  ...
  const auto differentiated = training::differentiate(
      program, build.loss_output, adapter_parameters);
  ...
  // Adapter-only AdamW: F32 parameters, F32 gradients (landed through the
  // Cast boundary in mixed mode), F32 moments ALWAYS.
    ...
    build.program.operations.push_back(
        {next_operation++,
         ...
         {ir::Attribute::f64(ir::AttrKey::LearningRate,
          ...
          ir::Attribute::f64(ir::AttrKey::Epsilon, config.epsilon_adam),
